...
window.generarSiguienteFolio = async function () {
    ...
    try {
        // Usamos un documento de contador diferente
        const docRef = doc(window.db, "config", "folios_devolucion");
        const docSnap = await getDoc(docRef);

        let siguiente = 0; // REGLA: El primero siempre será 0
        if (docSnap.exists() && docSnap.data().ultimoFolio !== undefined) {
            siguiente = docSnap.data().ultimoFolio + 1;
        }
...
window.guardarEnNube = async function () {
    ...
    // Apuntamos al nuevo documento de configuración
    const sfDocRef = doc(window.db, "config", "folios_devolucion");
    let folioAsignado;
    ...
    try {
        await runTransaction(window.db, async (transaction) => {
            ...
            let nuevoFolio = 0; // REGLA: Inicia en 0
            if (sfDoc.exists() && sfDoc.data().ultimoFolio !== undefined) {
                nuevoFolio = sfDoc.data().ultimoFolio + 1;
            }
            ...
            const valeData = {
                folio: folioAsignado,
                idTerceros: document.getElementById('idTerceros').value, // NUEVO CAMPO
            ...
            // Guardamos en una colección nueva para no mezclar con salidas
            const valesRef = collection(window.db, "vales_devolucion");
        ...
        console.log("Vale de devolución guardado con éxito. Folio asignado:", folioAsignado);
...
window.cargarHistorialDesdeNube = async function () {
    ...
    try {
        const q = query(collection(window.db, "vales_devolucion"), orderBy("timestamp", "desc"));
...
window.eliminarVale = async function (id) {
    ...
    try {
        await deleteDoc(doc(window.db, "vales_devolucion", id));
...
window.exportarPDF = async function () {
    ...
    // 1. Encabezado principal (Cambiamos título y color a Naranja #E67E22 para diferenciar)
    doc.setFillColor(30, 30, 30);
    doc.rect(0, 0, 210, 28, 'F');
    doc.setTextColor(230, 126, 34); // Color naranja/ámbar
    doc.setFontSize(20);
    doc.setFont("helvetica", "bold");
    doc.text("SOMSI - VALE DE DEVOLUCIÓN", 15, 18);
    ...
    // Inyectamos el ID de Terceros
    doc.text(`ID VALE XOFTMAQ: ${document.getElementById('idTerceros').value.toUpperCase()}`, 15, 37);
    doc.text(`FOLIO: ${document.getElementById('folioVale').value}`, 160, 37);

    doc.text(`TÉCNICO: ${document.getElementById('tecnicoNombre').value.toUpperCase()}`, 15, 44);
    doc.text(`FECHA/HORA: ${new Date().toLocaleString()}`, 145, 44);

    doc.text(`SUPERVISOR: ${document.getElementById('supervisorNombre').value.toUpperCase()}`, 15, 51);

    doc.text(`MARCA: ${document.getElementById('equipoMarca').value.toUpperCase()}`, 15, 58);
    doc.text(`ECONÓMICO: ${document.getElementById('equipoEco').value.toUpperCase()}`, 80, 58);
    doc.text(`SERIE: ${document.getElementById('equipoSerie').value.toUpperCase()}`, 145, 58);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.tarjeta-nav').forEach(link => {
        link.addEventListener('click', (e) => {
            if (formularioModificado) {
                e.preventDefault();
                alert("⚠️ Tienes datos ingresados sin guardar.\n\nPara cambiar de vale, primero guarda la información o limpia el formulario.");
            }
        });
    });
