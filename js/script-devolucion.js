/* --- SOMSI - SISTEMA DE VALES PRO (DEVOLUCIÓN) --- */
...
window.guardarEnNube = async function () {
    ...
    try {
        await runTransaction(window.db, async (transaction) => {
            ...
            const valeData = {
                folio: folioAsignado,
                idTerceros: document.getElementById('idTerceros')?.value || "",
...
window.cargarValeEnPantalla = function (docId, v) {
    ...
    if (document.getElementById('idTerceros')) {
        document.getElementById('idTerceros').value = v.idTerceros || "";
    }
...
window.exportarPDF = async function () {
    ...
    doc.autoTable({
        ...
        body: rows,
        headStyles: { fillColor: [20, 20, 20], textColor: [230, 126, 34] },
        ...
        if (listaOrdenada.length === 0) {
            ...
            listaOrdenada.forEach(item => {
                ...
                tr.innerHTML = `
                    ...
                    <td style="padding:8px; text-align:center; font-weight:bold; color:#e67e22;">${item.totalCant}</td>
                `;
                tbody.appendChild(tr);
            });
        }
...
window.evaluarEstadoFormulario = function () {
    const camposPrincipales = ['tecnicoNombre', 'supervisorNombre', 'equipoMarca', 'equipoEco', 'equipoSerie', 'idTerceros'];
...
window.exportarExcelHistorial = function () {
    ...
    XLSX.utils.book_append_sheet(libro, hoja, "Historial_Devoluciones");

    const fechaHoy = new Date().toISOString().split('T')[0];
    XLSX.writeFile(libro, `Reporte_Devoluciones_SOMSI_${fechaHoy}.xlsx`);
